...
// Enum for all the Device Resolutions
public enum MyDeviceResolutions {Res_2_3=0, Res_3_4, Res_4_5, Res_9_15, Res_9_16, Res_10_16, Res_11_16, Res_19_5_9, Res_19_5_9_Pro};

public class MultiplePlatformPortingHandler : MonoBehaviour {
	
	// Static Objects for Helper Functions
	private static MultiplePlatformPortingHandler mInstance = null;
	public static MultiplePlatformPortingHandler Instance {	get { return mInstance; } }
	
	// Private Constants							 			//  2:3,   3:4,   4:5,   9:15,   9:16,   10:16,  11:16, 19.5:9, 19.5:9 Pro
	private float[] ArrOfRatiosToAdjustCameraSize_Portrait 	=   { 1.333f,  1.5f,  1.6f,  1.2f,  1.124f,  1.25f, 1.375f, 1.126f, 1.081f };
	private float[] ArrOfRatiosToAdjustCameraSize_Landscape	=   {  3.0f,  2.666f, 2.5f,  3.33f,  3.55f,   3.2f,  2.909f, 4.333f, 4.4f };
	...
	public MyDeviceResolutions CurrentLoadedDeviceResolution { get { return mCurrentDeviecResolution; } }
	public int GameWidth = 1024;
	public int GameHeight = 768;
	public bool IsPortraitMode = false;
	
	// Use this for initialization
	void Start () { if(mInstance != null && mInstance != this) { DestroyObject(gameObject); return; } 
		else { DontDestroyOnLoad(this); mInstance = this; DetectCurrentDeviceResolution(); } }	
	// Update is called once per frame
	void Update () { if(currentLoadedLevel != Application.loadedLevelName) MakeAdjustmentsForTheCurrentResolution(); }
	
	void MakeAdjustmentsForTheCurrentResolution()
	{
		currentLoadedLevel = Application.loadedLevelName;
		
		float factorToAdjust = (IsPortraitMode ? ArrOfRatiosToAdjustCameraSize_Portrait[(int)mCurrentDeviecResolution] :
			ArrOfRatiosToAdjustCameraSize_Landscape[(int)mCurrentDeviecResolution]);
		float cameraSize = (GameWidth / 32.0f) / factorToAdjust;
		cameraSize = cameraSize*100.0f;
		cameraSize = Mathf.FloorToInt(cameraSize) / 100.0f;
		Debug.Log("Adjust Camera:"  +(GameWidth / 32.0f) + " / " + factorToAdjust);
		Debug.Log("Adjust Camera result:"  + cameraSize);
...
//				camera.rect = new Rect((1.0f - cameraViewPortWidth)/2.0f, 0, cameraViewPortWidth, 1.0f);
			}
		}
	}
	...
	void DetectCurrentDeviceResolution()
	{
		...
		Debug.Log("screenWidthByHeightRatio : "+screenWidthByHeightRatio);
		Debug.Log("screenWidthByHeightRatioInt : "+screenWidthByHeightRatioInt);
		
		bool isResolutionChanged = false;
		for(int i=0; i< ArrOfResolutionFactor_Landscape.Length; i++)
			if(screenWidthByHeightRatioInt == ArrOfResolutionFactor_Landscape[i] || screenWidthByHeightRatioInt == ArrOfResolutionFactor_Portrait[i]) 
		{mCurrentDeviecResolution = (MyDeviceResolutions)i;	isResolutionChanged = true; break;}
		...
		if(!isResolutionChanged)
		{
			...
			for(int i=0; i< ArrOfResolutionFactor_Portrait.Length; i++)
			{
				float value_ = Mathf.Abs(screenWidthByHeightRatioInt - 
					(Screen.height>Screen.width ? ArrOfResolutionFactor_Portrait[i] : ArrOfResolutionFactor_Landscape[i]));
		...
		Debug.Log("width y height y mCurrentDeviecResolution : "+ Screen.width+":"+Screen.height+" - "+mCurrentDeviecResolution);
		
		if(GetComponentInChildren<TextMesh>() != null)
		GetComponentInChildren<TextMesh>().text = Screen.width+"\n"+Screen.height+"\n"+mCurrentDeviecResolution;
	}
	
	public float GetPositionBasedOnDeviceResolution(float res_2_3, float res_3_4, float res_4_5, float res_9_15, float res_9_16, float res_10_16, float res_11_16, float res_19_5_9, float res_19_5_9_Pro)
	{
