...
public class UVAnimation
{
	...
	public bool GetFirstFrame(ref Vector2 uv)//cAjit
	{
		if(frames.Length > 0)
		{
			uv = frames[0];
	...
	public Vector2[] BuildUVAnim(Vector2 start, Vector2 cellSize, int cols, int rows, int totalCells, float fps)//cRoland
	{
		int cellCount = 0;
	...
	//cRoland
	public void appendFrame(Vector2 start, Vector2 cellSize)//cRoland
	{
		Vector2[] t_frames = new Vector2[1];		
		framerate = 10;
		t_frames[0] = start;
 		//t_frames[1].x = start.x + cellSize.x;
		//t_frames[1].y = start.y - cellSize.y;
		
		Vector2[] tempFrames = frames;
		if(frames != null)
		{
			frames = new Vector2[frames.Length + t_frames.Length];
			tempFrames.CopyTo(frames, 0);
			t_frames.CopyTo(frames, tempFrames.Length);
		}
		else
		{
			frames = new Vector2[t_frames.Length];
			t_frames.CopyTo(frames, 0);
		}	
	}
	...
	{
		//ADD NEW FRAMES TO EXISTING LIST
		Vector2[] tempFrames = frames;
...
public class SpriteManager : MonoBehaviour 
{
	...
	public int allocBlockSize = 1;				// How many sprites to allocate space for at a time. ex: if set to 10, 10 new sprite blocks will be allocated at a time. Once all of these are used, 10 more will be allocated, and so on...
	...
	public Sprite AddSprite(GameObject client, int leftPixelX, int bottomPixelY, int pixelWidth, int pixelHeight, bool billboarded)
	{
		return AddSprite(client, pixelWidth, pixelHeight, PixelCoordToUVCoord(leftPixelX, bottomPixelY), PixelSpaceToUVSpace(pixelWidth, pixelHeight), Vector3.zero, billboarded);
	}
