...
public class CChangeColorGUITexture : CAction
{
	public RawImage _TargetTexture = null;
	...
	public static CChangeColorGUITexture create(RawImage pTexture, Color pEndColor,float pDuration)
	{
		GameObject actionObject = new GameObject();
		actionObject.name = "CChangeColorGUITexture";
		
		CChangeColorGUITexture action = actionObject.AddComponent<CChangeColorGUITexture>();
		action.actionWith(pTexture, pEndColor, pDuration);
		
		action.autoReleaseCallBack = action.autoReleaseAfterCompletion;
		return action;
	}
	
	public void actionWith(RawImage pTexture, Color pEndColor,float pDuration)
	{
		_DestinationColor = new Vector4(pEndColor.r,pEndColor.g,pEndColor.b,pEndColor.a);
		_duration = pDuration;
		_TargetTexture = pTexture;
	}
	...
	override public void runAction()
	{
		...
		Color tStartColor = _TargetTexture.color;
