...
public class CRotateBy : CAction
{
	...
	public Vector3 _Rot = new Vector3(0,0,0);
	...
	public static CRotateBy create(GameObject pTargetObject, Vector3 pRot,float pDuration)
	{
		GameObject actionObject = new GameObject();
		actionObject.name = "CRotateBy";
		
		CRotateBy action = actionObject.AddComponent<CRotateBy>();
		action.actionWith(pTargetObject, pRot,pDuration);
		
		action.autoReleaseCallBack = action.autoReleaseAfterCompletion;
		return action;
	}
	
	public void actionWith(GameObject pTargetObject, Vector3 pRot,float pDuration)
	{
		_Rot = pRot;
		_duration = pDuration;
	...
	override public void runAction()
	{
		...
		_StartRot = mGameObjTransform.eulerAngles;
		_DestinationRot = _StartRot + _Rot;
